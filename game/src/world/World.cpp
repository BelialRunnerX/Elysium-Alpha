...
constexpr PlanetEnvironment kFrozen{
    "Frozen", "Cryovolcanic Frontier", 0.35f, 0.35f,
    {150, 178, 210, 255}, {210, 224, 232, 255}
};
constexpr PlanetEnvironment kToxic{
    "Toxic", "Caustic Biosphere", 1.1f, 0.45f,
    {72, 108, 56, 255}, {118, 156, 74, 255}
};
constexpr PlanetEnvironment kIrradiated{
    "Irradiated", "Rad-Storm Waste", 0.7f, 0.7f,
    {56, 72, 46, 255}, {140, 168, 72, 255}
};
constexpr PlanetEnvironment kOceanic{
    "Oceanic", "Pelagic World", 0.2f, 0.15f,
    {64, 116, 166, 255}, {120, 168, 186, 255}
};
constexpr PlanetEnvironment kAnomalous{
    "Anomalous", "Imperial Exclusion Zone", 0.9f, 0.8f,
    {36, 24, 52, 255}, {88, 64, 110, 255}
};
...
int World::generatedHeight(int x, int z) const {
    ...
    if (class_ == PlanetClass::Frozen) { base = 9.0f; amplitude = 4.5f; }
    if (class_ == PlanetClass::Toxic) { base = 8.5f; amplitude = 5.5f; }
    if (class_ == PlanetClass::Irradiated) { base = 7.5f; amplitude = 4.8f; }
    if (class_ == PlanetClass::Oceanic) { base = 6.0f; amplitude = 2.8f; }
    if (class_ == PlanetClass::Anomalous) { base = 8.0f; amplitude = 6.5f; }

    int h = static_cast<int>(std::round(base + (n - 0.5f) * 2.0f * amplitude));
    if (class_ == PlanetClass::Anomalous) {
        const float ridge = valueNoise2D(seed_, static_cast<float>(x), static_cast<float>(z), 5, 0x4E554C4CULL);
        h += static_cast<int>(std::round((ridge - 0.5f) * 4.0f));
    }
            ...
            for (int y = 0; y < Height; ++y) {
                ...
                if (y <= h) {
                    if (class_ == PlanetClass::Temperate) {
                        ...
                        type = (y == h) ? BlockType::Regolith : ((y >= h - 2) ? BlockType::Dirt : BlockType::Stone);
                    ...
                    } else if (class_ == PlanetClass::Anomalous) {
                        type = (y >= h - 1) ? BlockType::Basalt : BlockType::Stone;
                    } else {
                        type = (y >= h - 2) ? BlockType::Basalt : BlockType::Stone;
                    }
                    ...
                    if (y < h - 2) {
                        ...
                        const float oreBoost = (class_ == PlanetClass::Barren || class_ == PlanetClass::Irradiated) ? 1.35f : 1.0f;
                        if (ore < 0.012f * oreBoost && y < 9) type = BlockType::IronOre;
                        else if (ore < 0.026f * oreBoost && y < 12) type = BlockType::TinOre;
                        else if (ore < 0.048f * oreBoost && y < 14) type = BlockType::CopperOre;
                        else if (ore < 0.078f * oreBoost && y < 17) type = BlockType::CoalOre;
                    }
                    ...
                    if (class_ == PlanetClass::Anomalous && y <= 3) {
                        const float rift = hash01(seed_, x, y, z, 0x52494654ULL);
                        if (rift < 0.12f) type = BlockType::Magma;
                    }
                }
