...
import com.elysium.core.item.ElysiumArmorAscension;
...
public class ReforgeTableMenu extends AbstractContainerMenu {
    ...
    public void performAction(Player player) {
        ...
        // Reforging and ascension are armour operations.
        if (!(armor.getItem() instanceof ElysiumArmorItem)) {
            return;
        }

        // 2. Ascend two matching pieces.
        if (ElysiumArmorAscension.canAscend(armor, material)) {
            ItemStack ascended = ElysiumArmorAscension.ascend(armor, material);
            if (!ascended.isEmpty()) {
