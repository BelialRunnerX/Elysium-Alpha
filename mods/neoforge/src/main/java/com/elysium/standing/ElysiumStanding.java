package com.elysium.standing;

import com.elysium.Elysium;
import com.elysium.event.ElysiumPassiveHandler;
...
public final class ElysiumStanding {
    ...
    public static boolean addFavor(Player player, int delta) {
        if (delta > 0) {
            delta = scale(delta, ElysiumPassiveHandler.favorScale(player));
        }
    ...
    public static boolean addSuspicion(Player player, int delta) {
        if (delta > 0) {
            delta = scale(delta, ElysiumPassiveHandler.suspicionScale(player));
        }
    ...
    private static float spawnChance(int value) {
        if (value < NOTICE) {
            return 0.0F;
        }
